...
- Hovering a blocked row shows the clashing job, customer and overlap.
- One user per job — picking someone replaces the current assignee.
- Changing the job's start/end time re-checks availability; an assignee who now clashes is flagged on the chip.
